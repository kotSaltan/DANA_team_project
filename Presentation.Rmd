...
plot_clusters_on_map(355, hotspots_peak, zoom_level = 10)
...
ggplot(hotspots_peak_clean, aes(x = factor(year), y = ros)) +
  geom_boxplot(fill = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "ROS Distribution Across Years (Clean)",
       x = "Year",
       y = "ROS") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
...
ggplot(hotspots_peak, aes(x = hfi)) +
  geom_histogram(fill = "skyblue", color = "black", alpha = 0.7) + # Each bin represent a range of 1000 HFI units.
...
```{r, fig.width=7, fig.height=11, echo=FALSE}
plot_clusters_on_map(3983, hotspots_peak, zoom_level = 10) 

plot_clusters_on_map(4024, hotspots_peak, zoom_level = 10) 

plot_clusters_on_map(4094, hotspots_peak, zoom_level = 10)

plot_clusters_on_map(11877, hotspots_peak, zoom_level = 10)
```
...
ggplot(hotspots_peak_clean, aes(x = factor(year), y = hfi)) +
  geom_boxplot(fill = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "HFI Distribution Across Years (Clean)",
       x = "Year",
       y = "HFI") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
...
create_summary_table <- function(test_results) {
  test_summary <- data.frame(
    ...
    Y_2018_Mean = round(summary_2018$Mean, 2),
    Overall_Median = round(summary_overall$Median, 2),
    Y_2018_Median = round(summary_2018$Median, 2),
