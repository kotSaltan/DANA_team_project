...
```{r maps_top, fig.width=7, fig.height=11, echo=FALSE, warning=FALSE}
...
*The summary statistics shows the number of missing values, minimum, median, mean, and maximum values for each index.*

*During EDA we looked into distributions of six key Fire Indices across different years.*

*\
The distributions of these indices for a particular year, 2018, appeared to be close to the overall mean values of these indices.*
...
*Despite the higher number of fire events, the main indices (FFMC, DMC, DC, ISI, BUI, FWI) remained close to their mean values across the dataset.*
...
We also calculated the standard deviations for the fire indices to see the variability of these indices in 2018 compared to the overall dataset.
...
sd_summary <- data.frame(
  Index = fire_indices,
  Overall_SD = round(sd_overall, 2),
  Year_2018_SD = round(sd_2018, 2)
)
...
*Overall, the lower standard deviations for the year 2018 across all six indices compared to the overall dataset indicate that the conditions related to fire behavior and potential were more consistent in 2018.*

*Check the histograms if they follow the normal distribution.*

```{r, echo=FALSE}
# FFMC Histogram in 2018 with Normal Curve
ffmc_plot <- ggplot(hotspots_test_2018, aes(x = ffmc)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean(hotspots_test_2018$ffmc, na.rm = TRUE), 
                ...
                color = "red", size = 1) +
  labs(title = "FFMC Histogram in 2018",
       x = "FFMC",
       y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
...
dmc_plot <- ggplot(hotspots_test_2018, aes(x = dmc)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean(hotspots_test_2018$dmc, na.rm = TRUE), 
                ...
                color = "red", size = 1) +
  labs(title = "DMC Histogram in 2018",
       x = "DMC",
       y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
...
dc_plot <- ggplot(hotspots_test_2018, aes(x = dc)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean(hotspots_test_2018$dc, na.rm = TRUE), 
                ...
                color = "red", size = 1) +
  labs(title = "DC Histogram in 2018",
       x = "DC",
       y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
...
bui_plot <- ggplot(hotspots_test_2018, aes(x = bui)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean(hotspots_test_2018$bui, na.rm = TRUE), 
                ...
                color = "red", size = 1) +
  labs(title = "BUI Histogram in 2018",
       x = "BUI",
       y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
...
grid.arrange(ffmc_plot, dmc_plot, dc_plot, isi_plot, bui_plot, fwi_plot, ncol = 2)
...
```{r}
# Perform t-tests
test_results_t_test <- lapply(fire_indices, function(index) {
  t.test(hotspots_peak_clean[[index]], hotspots_test_2018[[index]])
})
test_results_t_test
```

```{r}
# Print summary table for t-test
summary_table_t_test <- create_summary_table(test_results_t_test)
print(summary_table_t_test)
```

*All the p-values are extremely small, indicating that the differences in means for all six indices between the overall dataset and the year 2018 are statistically significant.*

*This suggests that the conditions in 2018 were indeed different from the overall conditions, which could explain the increased number of fire events that year.*
...
Analyze the weekly number of fires using various fire index predictors.
The goal is to build a linear regression model to predict the number of fires and see its performance.
